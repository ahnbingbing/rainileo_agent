...
> 상태: **Phase 0 — 3-arm 룩 PD 승인 완료(2026-09-09, "굿"). Phase 1(커밋+배선) 대기** · 시작 2026-08-07
...
## ★ 전략 분기 (PD 2026-09-08) — 이 계획의 상위 프레임
- **이 3-arm 편집 문법 = 틱톡 Shorts 전략.** 틱톡은 계속 숏츠로 밀되, 이렇게 임팩트 있게 교차편집한다. `tiktok_expansion_plan.md`와 짝.
- **유튜브 = 롱폼으로 간다.** story(페이오프-선공개) 문법이 그 씨앗 — 지금은 숏츠 길이로 증명하고, 나중에 유튜브 롱폼으로 확장.
- 즉 같은 소재를 **틱톡=임팩트 숏츠 / 유튜브=롱폼**으로 갈래친다.
...
4. **클럽 = 컷마다 색이 바뀐다**(클럽 조명처럼). 등속 비트를 처음부터 끝까지 = 클럽도 코미디도 아님. **단 색은 hue-로테이션 금지 — 피사체(개·고양이)까지 색이 돌아 "초록 강아지"처럼 어색해진다(PD 9/8: "어중간하게 넣을 거면 더 강렬하게").** 대신 `colorbalance`로 그림자/미드/하이라이트를 클럽색으로 밀어 **빛의 캐스트**로 물들인다 — 피사체는 읽히되 장면이 네온으로, 컷마다 대담하게 바뀐다(`impact_edit.CLUB` 8색 팔레트).
5. **"이게 숏츠에서 먹히냐"는 아무도 모른다** → 그래서 **MAB로 실측**한다. 내가 한 방향을 답인 양 들이밀지 않는다.
6. **story = 페이오프-선공개(PD 9/8).** "잔잔한 ASMR 심리스 루프"가 아니라, **결과(클라이맥스)를 맨 앞 콜드오픈**으로 던지고("이게 무슨 일이냐면…") → 되감기 → 빌드 → 페이오프로 복귀하는 리텐션-훅 서사. 무음·비구독자 첫 1초에 페이오프를 먼저 보여줘 "어쩌다?"를 심는 구조.
...
| arm | 길이 | 편집 문법 | 차별 축 |
|---|---|---|---|
| **A. velocity (=클럽)** | 15~20s | 비트에 컷, 드랍에 속도램프(슬로우→스냅), **컷마다 컬러 캐스트(colorbalance 클럽조명)**, 플래시 | 고에너지·재시청 |
| **B. meme / reaction** | 15~20s | 점프컷, **줌 펀치**(tighter crop), **프리즈 프레임**, 큰 박스 키네틱 자막(펀치라인), 붐/딩 효과음 | 코미디 타이밍 |
| **C. story (페이오프-선공개)** | ~35~45s(숏츠) → 유튜브 롱폼 확장 | **클라이맥스 콜드오픈 → 되감기 → 빌드 → 페이오프 복귀**, 시네마틱 그레이드 | 리텐션 훅·롱폼 씨앗 |
...
- **컷마다 색 변화** — ~~hue 로테이션~~ → **colorbalance 클럽조명 캐스트로 교체(9/8)**. hue는 피사체까지 색이 돌아 어색 → shadows/mids/highs를 클럽색으로 밀어 빛만 물들임(`CLUB` 8색). *구현·강렬화 완료.*
- **속도 램프** — 슬로우 비트(3비트)→드랍 버스트(1비트). *구현됨.* + meme 줌펀치(tighter crop)·프리즈 프레임(`render_freeze`) 구현.
- **키네틱 자막** — Pretendard-Black/ExtraBold, 세이프존, meme은 박스 배경(box=1). *구현됨(팝 애니메이션은 ffmpeg 세그폴트 회피로 고정크기 — 복구 예정).*
- **사운드 디자인** — 붐(56Hz)·딩(1040Hz) 스팅 제너레이터(`_gen_sfx`), 시각과 타임코드 정렬. *휘익/라이저 추가 예정.*
...
- ✅ **3-arm 룩 PD 승인 (2026-09-09 "굿")** — v5 기준:
