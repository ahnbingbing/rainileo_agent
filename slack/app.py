...
def _grandma_converse(client, channel, user, text, thread_ts, asset_id=None):
    ...
    try:
        ...
        sys_p = (
            ...
            "고마워요' 정도로 받아라.\n"
            "★이름/호칭 정확히: 우리가 아는 펫은 랴니(강아지)와 레오(고양이) 둘뿐이다. '하비' 등 그 외 "
            "이름은 사람인지 동물인지 함부로 단정하지 말고, 여럿을 '아이들'·'세 친구'·'펫들'로 뭉뚱그리지 "
            "마라(가족이 어색해한다). 가족이 부른 이름 그대로 부르고, 누구인지 모르면 중립적으로 받아라.\n"
            "★캐묻지 마라: 매 답마다 질문을 붙이지 말 것. 대체로 따뜻한 공감·맞장구로 끝내고, 질문은 정말 "
            "자연스러울 때만 가끔. 끼니·취침은 반복해 챙기라 하지 마라(잔소리처럼 들린다).\n"
