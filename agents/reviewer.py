...
- **지어낸 급발 사건·사운드로 만든 위기 서사 (real_footage, PD 2026-09-10)**: this is the intersection of the invented-EVENT rule (95) and the contradicted-emotion rule (96), called out because an OFF-SCREEN sound is easy to miss. When the caption/title builds its story on a sudden event or sound the frames cannot show — 천둥·번개·사이렌·초인종·폭발·경보(thunder/siren/doorbell/alarm), or the emergency it supposedly triggers (대피/방공호/도망/숨기/패닉/비상/SOS) — the ONLY valid evidence is the pet's REACTION: a real jolt reads on screen (귀 납작/움찔/튀어오름/눈 커짐/후다닥 숨음). The sound is off-frame but the startle is not. If the frames instead show the pets CALM (누워 있음·늘어짐·앉아 딴청·느긋) with no shock response, the whole alarm/panic narrative is fabricated — a lounging relaxed cat framed as "쾅! 천둥에 깜짝 → 소파 방공호 대피!" is exactly this lie → log in `caption_vs_clip_mismatches` and cap ≤5. Guard against false-positive: cap ONLY when the startle reaction is ABSENT from the frames — if you actually SEE the ears flatten / the body flinch / a scramble, the event is earned and must not be flagged.
- **랴니↔레오를 '첫 만남/처음 만난 사이'로 프레이밍 — 거짓 전제 (양레인, PD 2026-09-06)**: 랴니와 레오는
  2025년 가을 레오 합류 이후 한집에 사는 오래된 남매다(canon). title·theme·caption이 둘을 '첫 만남/첫 대면/
  처음 만난/서로 낯선/첫 인사/첫 관찰기'로 그리면(레오 합류 당시의 실제 첫날 영상이 아닌 한) 관계에 대한
  거짓이다 → log in `caption_vs_clip_mismatches` and cap ≤5. (Worked example: 하비가 사료를 굴려주고 둘이
  주워 먹는 클립을 "고양이와 강아지 첫 만남 관찰기"로 발행 — 둘은 이미 한집 식구다.) 관련 오독: 두 펫이
  바닥에 코를 박은 자세를 "서로 코인사/눈맞춤"으로 캡션하면 대개 바닥의 사료·냄새를 먹거나 맡는 것의 오독이다
  (서로 마주 보고 코가 맞닿을 때만 코인사) → 같은 ≤5.
...
- **A caption-vs-clip mismatch requires FRAME EVIDENCE — do NOT fabricate one (PD 2026-07-23, critical reviewer-accuracy fix, parallel to the sparse-sampling note in CHECK 0).** A `caption_vs_clip_mismatches` entry is ONLY valid when the frames you can SEE UNAMBIGUOUSLY CONTRADICT the caption — a claimed kick/motion/scene-change (94-95) or surface/place (99) that plainly isn't happening. Two false-positive patterns are FORBIDDEN because they wrongly emptied good slots:
  ...
  Default: if you are INFERRING absence/mismatch rather than SEEING a contradiction, it is NOT a mismatch. Give a present-pet caption the benefit of the doubt; reserve the mismatch flag (and its score cap) for the clear frame-visible lies above.
