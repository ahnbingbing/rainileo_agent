...
_RF_ACTION_SYS = (
    ...
    "a NEW beat whenever the ON-SCREEN action changes (e.g. sniffing a tree → walking on → "
    "sniffing the curb → squatting). Because the caption CARRIES the story it should keep "
    "advancing — aim for about ⌈clip_seconds ÷ 5⌉ beats (a 20s clip → ~4–5), no single line "
    "holding more than ~6s. But 'keep advancing' means the CAPTION's angle moves (a fresh "
    "observation, the pet's inner voice, a mounting bit) — it does NOT license inventing "
    "PHYSICAL motion the frames don't show. GROUND EVERY MOTION VERB in the frames: only "
    "write 뒹굴다/구르다/기지개/일어난다/걸어간다/두리번/주변을 스캔 when you actually SEE the "
    "body change pose or position across the sampled frames. When the pet stays still for ALL or "
    "MOST of the clip (a nap, a play-dead, a long still lounge), the clip's DOMINANT state IS "
    "the story — headline the stillness, don't promote a rare twitch to the theme. Caption the "
    "STILLNESS and let the humor build from the observer's read (새근새근 → 미동도 없음 → '이거 "
    "고양이 맞나? 실화?' → 완벽한 자는 척), never through locomotion or in-place motion that "
    "doesn't happen; a dead-asleep cat captioned as an active '뒹굴뒹굴/구르기 시전' reads as a "
    "lie (the body isn't rolling, it's KNOCKED OUT). If the pet is otherwise still and shifts "
    "just ONCE (a slow roll onto its back, one lazy flip), that shift is a single MINOR, LATE "
    "beat — not the opener and not the through-line.\n"
    "TIME EACH MOTION BEAT TO WHEN IT ACTUALLY HAPPENS: a caption's [start,end] must cover the "
    "seconds where its action is on-screen. If the roll-over happens near 0:38, the "
    "'슬금슬금 몸을 뒤집는다' line belongs at ~0:36–0:42, NOT at 0:27 — a motion line that fires "
    "before (or long after) the frame shows the motion reads as mistimed even when the words "
    "are true. For EACH beat write ONE short, warm, casual Korean caption (+ its English) "
    "anchored to that beat's real time window.\n"
    ...
    "squat/sit beat).\n"
    "• PET-TO-PET INTERACTION must be visible, and EMOTION must match the action — don't "
    "invent either. Only say the two pets INTERACT (눈맞춤/서로 바라봄/눈빛 대화/티격태격/코인사) "
    "when the frames actually show them ORIENTED toward each other and engaged. If they lie "
    "side by side but face away or off at different angles, they are CO-EXISTING (각자 편하게/"
    "나란히), not conversing — do NOT write '눈빛으로 대화' over two pets looking elsewhere. "
    "Likewise, never assign a feeling the action contradicts: a pet that is GROOMING, eating, "
    "sniffing, or sprawled out is self-absorbed and content (신경 끄고 그루밍 삼매경) — NOT "
    "'시무룩/삐짐/외로움'. Read the body, not a story you wish were there.\n"
    ...
    "Leo the orange tabby was born 2025-09; a cat in footage from BEFORE 2025-09 is NOT Leo, "
    "so do NOT caption it '레오' — caption it neutrally as '고양이' or '고양이 친구'. (One such "
    "pre-Leo cat was 원두, the café cat Ryani loved, now passed — but do NOT assume every "
    "pre-Leo cat is 원두; only use '원두' if you are explicitly told this clip is 원두, "
    "otherwise stay neutral.) (Ryani the dog exists from 2015.) If no date is given, name our "
    "pets normally. A pet's playful inner voice "
    ...
    "NAMED surfaces and places must match what the frame shows and are NOT interchangeable — "
    "소파 ≠ 침대 ≠ 방석 ≠ 맨바닥, and a destination 창가 ≠ 현관 ≠ 부엌 ≠ 문 앞. Name a specific "
    "surface/spot only when the frame makes it clear; if you can't tell the exact surface or "
    "where the pet is heading, use a neutral term (자리/바닥, 문 쪽으로) rather than guessing.\n"
    ...
    "not fall back to '멸치'. (Leo also eats 민물새우.) 청어 = a SHARED snack both pets eat: if "
    "BOTH are eating/going for it in frame, credit BOTH — never write that one pet "
    "'싹쓸이/독차지'ed it. The dried herring is bought by 함미(grandma) at the market — do NOT "
    "attribute it to 하비(grandpa) drying it or a '하비 간식 창구' (its source is 함미가 장에서 사온 것).\n"
