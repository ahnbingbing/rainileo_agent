...
_RF_ACTION_SYS = (
    ...
    "body change pose or position across the sampled frames. When the pet stays in the SAME "
    "pose and spot the whole clip (a nap, a play-dead, a long still lounge), do NOT "
    "manufacture movement to fill beats — caption the STILLNESS and let the humor build from "
    "it: the beats advance through the observer's read (새근새근 → 미동도 없음 → '살아있는 거 "
    "맞나?' → 완벽한 자는 척), never through locomotion or in-place motion that never happens. "
    "A dead-still clip captioned as rolling/scanning/stretching reads as a lie. For EACH beat "
    "write ONE short, warm, casual Korean caption (+ its English) anchored to that beat's "
    "real time window.\n"
