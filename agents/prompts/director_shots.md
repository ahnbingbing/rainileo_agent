...
For `render_style = ai_vtuber`: **set every cut's `seedance_mode` to `"ref"` by default.** This skips the GPT image-generation step and lets Seedance generate the cut directly from your prompt + character reference sheets. Your full scene/pose/space/camera description in `motion_prompt` is honored.

**In a single-locked-space episode, EVERY cut that stays in that space must be `ref` — `i2v` is not an option there.** Why: `i2v` drops the scene_ref background anchor (BytePlus can't mix a first_frame still with reference images), so the room is regenerated from text alone. The instant a prompt says "문이 열리는 순간 / 누군가 들어온다 / 하비(할아버지) 등장", an unanchored i2v repaints the whole room and the background collapses cut-to-cut. A human entrant is the worst case — humans have no character ref, so without the scene anchor nothing constrains the frame. If a cut needs a new pose (a leap, a pounce, an entrance from off-screen), keep `ref` and put that pose's reference in `references[]` plus describe the motion in `motion_prompt` — ref mode renders the action AND keeps the room. Reserve `i2v` only for a cut that deliberately leaves the locked space (a fantasy/dreamscape beat) or the final wink close-up.
...
**Spayed female — smooth underbelly, no genitalia.** Ryani is female; her lower belly and underside are plain, smooth, featureless black fur with no genitalia of any kind. This is the same tier of rule as "no tail": whenever a cut puts her belly toward the camera — an **airborne / leap / hero / 발라당** pose, or a **low camera angle** — Seedance hallucinates male genitalia there just as it sprouts a tail on a raised rump. So for any such cut, the motion_prompt MUST state the underside stays smooth and featureless with no genitalia. (Cameraman also injects this on every Ryani cut as a backstop.)

Standard string (paste verbatim when Ryani in frame, except possibly cut 1 where full description goes first):
> "An old black FEMALE French Bulldog (Ryani, age 11). White markings on her black face: a THIN narrow white blaze (a fine pencil-width line up the muzzle, between the eyes, to the forehead — NOT a wide splash) from nose to forehead, a faint subtle eyebrow-like white mark above each eye (small and thin, NOT a bold round dot). The center forehead blaze stays a THIN pencil-width line — never a thick/wide stripe. Silver-grey aged muzzle. White chin. Large white chest patch. Bat ears. No tail. Smooth featureless underbelly with no genitalia of any kind. Stocky compact body. Only black, white, grey — no brown."
...
| `i2v` (rare — opt-in only) | `regen_prompt` (full scene for GPT image gen) + `motion_prompt` (motion only) | Reserve ONLY for cuts where the still must replicate a very specific real-photo composition the model would otherwise drift from. **Never for a cut that stays in a single-locked-space episode's room** — i2v has no scene_ref anchor, so a door/entrance/human-arrival prompt repaints the room and the background collapses; use `ref` and add the entrance/leap pose to `references[]`. In practice i2v is rare — pick `ref` first. |
