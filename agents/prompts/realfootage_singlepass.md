...
**주체가 또렷이 "우리 애"로 읽히는 클립만 — 원거리·잡샷 배제:** 레오·랴니가 **크고 선명하게** 잡힌 클립을 골라라 — `comp`(composition)=medium/close-up + `focus`가 해당 펫 + `looking_at`=camera(표정이 살아있음). **주체가 멀고 작게 나온 원거리 샷(`comp`=overhead/wide/far)이나 배경이 잡다한(로봇청소기·잡동사니) 샷은 쓰지 마라 — 다른 close 클립이 정말 없을 때만 최후로.** 왜: 피사체가 너무 작고 멀면 그게 레오·랴니인지조차 안 읽혀 **"관계없는 아무 길고양이/강아지"처럼 보인다** — 이 두 마리가 주인공이라는 채널의 존재 이유가 무너진다. (실제 실패: 랴니가 멀찍이 작게 걸어가는 원거리 산책 클립 → PD "아예 관계없는 길고양이 같다".) 기준작 ep_20260519 / 115505처럼 펫 얼굴이 화면에 또렷이 들어오는 컷이 정답이다.
...
## 훅을 앞세워라 — 가장 볼거리 있는 컷이 먼저, 그리고 가장 길게 (가장 큰 누수)

리텐션은 초반(앞 3-5초)에 잡히지만 **이탈은 중반 이후**에 난다(~15초부터 평균 아래). 그래서 편집의 첫 판단은
"어떤 순서로 잇나"가 아니라 "**무엇을 앞세우나**"다.

- **가장 사건성 있는 비트를 맨 앞에 놓고, 그 비트에 가장 많은 시간을 줘라.** 클립마다 에너지가 다르다 —
  먹방·물놀이·놀이·간식 쟁탈·재롱처럼 **뭔가 벌어지는(사건) 컷**과, 자는·눕는·멍하니 앉은 **정적인 컷**.
  정적인 컷을 앞에 길게 깔고 사건 컷을 맨 뒤에 짧게 붙이면, 볼거리가 다 지나간 뒤에야 훅이 나와 이미 이탈한
  뒤다. **사건 컷이 리드(cut1)이고 가장 길다. 정적/여운 컷은 그 뒤를 짧게 받친다.** (실제 실패: 레오 청어
  먹방[사건]을 맨 뒤 10초에 묻고 바닥에 발라당 누운 정적 컷을 앞 30초로 깐 회차 → PD "청어를 앞으로 하고
  길게, 눕는 걸 뒤로". 먹방이 이 소재의 훅인데 뒤에 숨었다.)
- 연결조직("오늘도→한편→그런데")은 여전히 흐름을 잇되, **비트 순서는 서사 시간순보다 에너지순이 우선**이다 —
  필요하면 사건을 먼저 보여주고 그 앞의 정적 상태는 "~하기 전, 잠깐은 이랬어요"처럼 뒤에 짧게 받쳐도 된다.
  단, 왜곡 금지(충실 > 흐름)는 그대로 — 순서만 바꾸는 것이지 없는 사건을 지어내는 게 아니다.
- **페이오프 이후 빠르게 닫아라.** 끝의 여운은 RF 장르의 미덕이지만 **한 줄짜리 짧은 버튼**이다 — 같은
  잔잔한 비트를 뒤에 더 얹거나 freeze로 길게 늘이면 그게 새는 구간이다.
- 앞 3초는 이미 잘 된다 — 화력은 "사건을 앞세웠나"와 중후반 군더더기 제거에.
