...
- **Fixture-inside staging (PD 2026-06-08 — 욕실 세면대 바닥 사건)**: a motion_prompt/action stages a pet **standing or sitting INSIDE a wall-/vanity-mounted fixture** — sink basin(세면대), toilet(변기), wall-set bathtub, counter — as if it were on the floor (e.g. "Ryani stands inside the white square sink basin"). Seedance grounds the fixture (drops the sink onto the floor) to make the pose plausible → 세면대가 바닥에 놓인 그림. Tier 1 BLOCKER. fix_hint: pet stands on the floor/chair beside the fixture with paws lifted UP to the faucet, OR use an explicit floor 대야/washtub described as ON the floor. EXCEPTION: a genuine floor-level basin/대야/tub explicitly placed ON the floor is fine.
