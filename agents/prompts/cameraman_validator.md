...
- **Thin background description (PD 2026-06-08 — Seedance 비용 + 배경 약점)**: Seedance is expensive and BAD at backgrounds — it freelances/breaks the room unless the prompt over-specifies it. The concept's `set_description` (prepended to every cut) MUST be a VERY detailed, camera-sweep room spec — as if a camera slowly pans across the room naming each element one by one (walls → floor → furniture-with-positions → window/lighting → props). Tier 1 BLOCKER if set_description is thin: shorter than ~400 chars, OR missing any of {walls, floor, main furniture with positions, lighting}. fix_hint: rewrite set_description grounded in set_library[set_anchor].persistent_background + room_layout_3d, camera-sweep style, every surface/furniture/light enumerated. (Don't spend Seedance $ on a concept whose room will render broken.)
