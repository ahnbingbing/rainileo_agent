...
- **Physical impossibility**: cat eats from a sealed kibble bag without it opening; pet jumps from floor to a 2m surface in <1s; food appears from nowhere; objects pass through each other.
  - **⚠️ EXCEPTION — INTENTIONAL SURREAL HOOK (PD 2026-06-11): DO NOT block a cut just because it defies real-world physics WHEN the impossibility IS the deliberate gag.** ai_vtuber's signature fun is surreal fantasy: 랴니가 거실에서 수영, 세면대 물이 넘쳐 레오가 서핑/파도타기, pets floating, indoor rain/flood. If the title/beat/caption FRAMES the impossibility as a playful fantasy, it is the HOOK — approve it, do NOT mark "물리 불가능". The test: "Could an animator draw this ON PURPOSE as a fun gag?" YES → allow. This carve-out applies ONLY to deliberate, coherent fantasy. It does NOT excuse genuine CONTINUITY bugs (a pet in two places at once, food materializing as an unintended error, a sink grounded on the floor) — those still block. Surreal ≠ incoherent.
...
- **Contradicting character traits**: Leo eats crumbs off the floor (Ryani is the floor specialist). Ryani is given a tail. Leo "the elder" (he's ~10 months, a grown cat now — roughly Ryani's size, but still the younger one). Pets renamed or merged.
...
- **Thin background description (PD 2026-06-08; RELAXED PD 2026-06-12)**: ⚠️ PD 2026-06-12 — for **ai_vtuber the scene is carried by the regen IMAGE** (a concept-grounded Ryani+Leo still already placed in THIS scene), NOT by a long text `set_description`. So **do NOT Tier-1 BLOCK an ai_vtuber concept for a "thin" set_description**, and **NEVER apply the walls/floor/furniture/lighting checklist to an OUTDOOR or FANTASY scene** (beach, sky, ocean, fountain, space — it has no walls or furniture). At most a SOFT note (not a blocker) if an INDOOR-ROOM set_description is sparse. The ONLY hard block here is the truly-empty case: set_description essentially missing (< ~80 chars) AND no scene grounding at all. fix_hint (soft): add a few concrete scene anchors (ground/floor, horizon or sky, one key landmark, light direction) — a full camera-sweep room paragraph is NOT required. (The old 400-char camera-sweep room spec was an INDOOR-only guard that wrongly blocked beaches and out-of-room fantasy scenes.)
...
- **⚠️ ASSET rules are real_footage-ONLY (PD 2026-06-09)**: All `asset_id` / `asset_ground_truth` / asset-fidelity rules below apply ONLY when `render_style == "real_footage"`. **ai_vtuber concepts are GENERATED (Seedance ref/i2v) — they have NO real source asset, so the ABSENCE of `asset_id` / `asset_ground_truth` is EXPECTED and CORRECT, never a defect.** Do NOT block, revise, or penalize an ai_vtuber concept for "missing asset_ground_truth" / "no asset_id" / "all cuts seedance_mode=ref" — that is normal. (This was falsely blocking the launch AV self-intro every render.)
- **2+ asset-fidelity violations = BLOCK (PD 2026-06-03 escalation, real_footage ONLY)**: Previously asset-fidelity mismatches resulted in "revise" verdict. NOW: if 2 or more cuts have actions inventing objects/scenes NOT in their `asset_ground_truth.sc`, return `verdict: "blocked"` (not "revise"). Real_footage demands cut-by-cut accuracy — partial fidelity is not acceptable. One mismatched cut MAY be tolerated; two or more = the concept is fundamentally broken.
...
- **Causal chain break — cause must be ON SCREEN (PD 2026-06-13 승격, was Tier 2)**: cut N's action/location/state changes from cut N-1 WITHOUT the causing event being visible in any cut. The test: **"watching the video with captions OFF, can a viewer tell WHY this changed?"** A causal connective in the caption ("그래서/결국/하지만/그러다") does NOT satisfy this — the CAUSE must be staged as a visible action in the preceding cut. Real failure (2026-06-13 무더위): cut2 "랴니 분수에서 신남" → cut3 "그래서 결국 집으로" — the cause (더위 싫은 레오가 랴니를 말리는 사건) was never a cut, so the return home is unexplained. Tier 1 BLOCKER. fix_hint: name the missing cause and require an added cut/action staging it (e.g. "add a cut: Leo presses a paw on Ryani, tugging her from the fountain — '집에 가자' beat — before the home cut").
- **PD-directive beat contract (PD 2026-06-13 NON-NEGOTIABLE)**: when the input includes a PD directive (`arc_directive`/`pd_directive`) that enumerates a beat sequence (①②③… or [컷1]…[컷N] style), EVERY enumerated beat must map to its own cut with the SAME core action and actor. Tier 1 BLOCKER when any directive beat is missing, merged into another cut, or its core action is swapped (e.g. directive says "레오가 랴니를 말림" but no cut stages Leo restraining). Cut-count guidance (short=3-4) NEVER justifies dropping a directive beat — more beats → more cuts. List the missing beat(s) verbatim in the issue description.
...
- **Weak causal TONE (soft)**: cuts connect but the bridge is thin (no sensory bridge phrase, abrupt rhythm). NOTE: a missing on-screen CAUSE for a location/state change is NOT this — that is the Tier-1 "Causal chain break" above.
