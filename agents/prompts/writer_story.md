...
- **초현실 훅엔 인과(why)를 캡션으로 먼저 세워라 (PD 2026-06-11, 필수)**: 거실 수영,
  ...
  - **상상 컷엔 명시 라벨도 달아라**: setup 인과에 더해, 상상 장면 컷 자체에 "레오의 상상
    ...
    구분돼야 한다.
- **테마는 화면으로 전달돼야 한다 — 캡션-only 테마 금지 (PD 2026-06-24)**: "카지노/잭팟",
  "무대", "우주" 같은 컨셉 테마를 캡션·제목에만 적고 화면은 그냥 펫만 나오면 시청자에겐
  "스타일 불일치"다(잭팟 회차가 이 때문에 Giri 6/10). 핵심 테마 비주얼은 ① 실제 렌더로
  보여줄 수 있는 것으로 잡거나(소품·배경·동작), ② Seedance가 못 만드는 화려한 그래픽
  (잭팟 사인·폭죽·네온)은 **렌더 후 오버레이 효과로 입힐 것을 전제로** 컨셉에 명시하라
  (Director/Cameraman이 합성). 화면이 못 받쳐줄 테마를 캡션으로만 약속하지 마라.
