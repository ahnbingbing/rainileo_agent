...
2. **영상 — 안 튀는 연속성**: 모든 컷은 같은 set_anchor·같은 캐릭터·chain_from_prev로 이어져
   앞 컷의 끝이 다음 컷의 시작으로 자연스럽게 흐른다. 컷이 많을수록 인접 컷의 동작·위치·시선을
   연결해 점프컷을 피한다. 공간이 바뀌면 short 불가 → mid.

### 컷 메커닉

- cut 1 = ref mode(char refs + scene_ref + R2V), cut 2+ = chain i2v(cameraman 자동 처리).
