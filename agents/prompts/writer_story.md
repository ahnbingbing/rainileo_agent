...
- **컷 수 제한 없음 (PD 2026-06-14).** 컷 수는 *이야기의 비트 수*가 정한다 — 필요하면
  8컷, 10컷+ 도 OK. 한 set_anchor 안에서. 컷 간 카메라 POV는 살짝 변경 OK (눈 클로즈업 →
  발 클로즈업 같은 micro shift), 큰 공간 전환은 mid로 격상.
- **컷당 길이는 목표 길이에 맞춰 줄인다 (가변).** ~25초에 비트가 많으면 **컷을 짧게**:
  예) 10컷 × 2.5초 = 25초, 6컷 × 4초 = 24초. "각 컷 5초 고정"은 폐기 — 이야기에 맞춰
  촘촘하게. `duration_seconds`는 2.5~6초 범위에서 컷별로 자유롭게(비트가 빠르면 짧게,
  여운이 필요하면 길게). 킥/엔딩 컷은 조금 더 길게 잡아 여운.
