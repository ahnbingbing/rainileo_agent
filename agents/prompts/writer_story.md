...
|---|---|---|---|
| **`short`** | **이야기 비트 수만큼 (상한 없음 — 4컷도, 10컷+도)** | ~25-30초 (with 범퍼) | **같은 공간(set_anchor) 안에서 짧은 컷 여러개를 chain**. Cut 1 = ref mode. Cut 2+ = i2v with prev cut's last ffmpeg-extracted frame. 컷당 길이는 가변(2.5~6초) — 비트 많으면 컷을 잘게 쪼개 다 담는다(예 10컷×2.5초). 컷 간 continuity는 frame chain 자동 락. 컷별 자체 캡션. |
...
- **컷 수는 비트 수가 정한다, 단 가독성 상한 안에서 (PD 2026-06-14).** 3-5컷 고정 폐기 —
  필요하면 영상 컷 ~8개까지(그 이상의 밀도는 이미지 몽타주로, 아래). 한 set_anchor 안에서.
  컷 간 카메라 POV는 살짝 변경 OK (눈 클로즈업 → 발 클로즈업 같은 micro shift), 큰 공간
  전환은 mid로 격상.
- **컷당 길이는 목표 길이에 맞춰 줄인다 (가변).** ~25초에 비트가 많으면 **컷을 짧게**:
  예) 8컷 × 3초 = 24초, 6컷 × 4초 = 24초. "각 컷 5초 고정"은 폐기 — 이야기에 맞춰 촘촘하게.
  `duration_seconds`는 2.5~6초 범위에서 컷별로 자유롭게(비트가 빠르면 짧게, 여운이 필요하면 길게).
  킥/엔딩 컷은 조금 더 길게 잡아 여운.
- **영상 컷 ≠ 이미지. 가독성이 진짜 상한 (PD 2026-06-14).** 영상(i2v) 컷은 캡션을 읽고
  모션을 이해할 시간이 필요해 **컷당 2.5초 미만 금지** → 25초 short는 실질 **영상 컷 ~8개
  내외가 상한**(20초에 20컷 같은 건 과해, 캡션도 영상도 못 읽음). **반대로, 빠른 사진 몽타주
  (photo_sequence, 스틸 이미지)는 20초에 20장도 OK — "오히려" 적합**(스틸은 1장당 짧아도
  읽힘). 즉 "촘촘하게"의 정답은 *영상 컷을 무한정 늘리는 게 아니라* 가독성을 지키며(캡션·영상·
  텍스트 모두 잘 읽히게) 이야기를 압축하는 것. 밀도가 더 필요하면 영상 컷 수가 아니라
  이미지 몽타주를 쓴다.
- **스틸 이미지 컷은 켄번스 필수 (PD 2026-06-14).** 스틸(사진)을 쓸 땐 둘 중 하나:
  (a) **duration을 짧게** 해서 빠르게 넘기거나, (b) duration을 길게 잡으려면 **반드시 줌인/
  줌아웃(또는 팬) 켄번스 카메라 무빙을 넣어** 정지화면이 얼어붙지 않게 한다. 길게 멈춘 정지
  프레임 금지 — 죽은 화면이 된다. (Director가 `camera_move`에 zoom_in_slow/zoom_out_slow/
  pan을, 짧게 갈 컷은 짧은 `duration_seconds`를 부여; photo_sequence 렌더가 켄번스+xfade 처리.)
- **풀 인과 아크**: 각 컷 = 하나의 인과 비트(기→승→전→결). "한 비트 갸그"로 납작하게
  만들지 말 것. 킥은 마지막 비트에 터진다. 비트가 많을수록 컷을 더 잘게 쪼개 다 담아라.

⚠️ **자유 = 품질 책임 (PD 2026-06-14 — 컷/길이 제약을 풀어주는 조건):** 예전에 컷 수·길이를
3-5컷×5초로 묶었던 건, 자유를 주면 (a) **말도 안 되는 이야기를 늘어놓고**(sprawl) (b) **영상이
여기저기 막 튀어서**(컷 간 연속성 붕괴)였다. 제약을 푸는 대신 그 두 가지를 *반드시* 막아라:
  1. **이야기**: 컷 수는 *실화에 실제로 있는 비트*만큼만. 늘리려고 없는 사건을 지어내거나
     군더더기 비트를 끼우지 마라. 모든 컷이 인과 사슬에서 빠지면 안 되는 필수 비트여야 한다
     (빼도 이야기가 되면 그 컷은 군더더기 → 삭제). 한 줄 로그라인으로 전체가 설명돼야 한다.
  2. **연속성(안 튀게)**: 모든 컷은 같은 set_anchor·같은 캐릭터·chain_from_prev로 이어져
     **앞 컷의 끝 = 다음 컷의 시작**이 자연스러워야 한다. 컷이 많아질수록 점프컷처럼 튀지
     않도록 인접 컷의 동작/위치/시선이 연결돼야 한다. 공간이 바뀌면 short 금지(mid).
  → 요구되는 건 **이야기 퀄리티 + 영상 coherence**다(해상도는 별개 — lo-fi 허용).
- 다공간이면 무조건 `mid`로 격상 (이건 공간 제약이지 이야기 제약이 아님).
- cut 1은 ref mode (char refs + scene_ref + R2V), cut 2+ 는 chain i2v (자동, cameraman이 처리).
- 각 컷에 자체 captions[]. scene 분할은 그 컷 길이 안에서.

**1분 동영상에 단일 갸그면 시청자 이탈** — mid를 고를 거면 그 긴 호흡을 채울 실제
빌드업이 있어야 한다. 단, "이야기가 풍부 = mid" 가 아니다 (아래).

**사건/twist 스토리도 short로 압축 가능 (PD 2026-06-14 — 2026-06-02 룰 개정):**
- 예전 룰("사건 스토리는 무조건 mid, 25초에 기승전결 욱여넣지 마라")은 **폐기**한다. PD:
  "short와 mid 차이는 영상 길이지 이야기 길이가 아냐. 25초라도 더 많은 이야기를 압축할 수
  있잖아." → 풀 인과 아크(기→승→전→결)+킥을 **25초에 압축해 담는 것이 정상이자 목표.**
- viewer가 "갑자기 만나고 끝나네?" 라고 느끼는 진짜 원인은 *짧은 길이*가 아니라
  **비트를 빼먹었거나, 인과 연결이 약하거나, 킥까지의 빌드업이 없어서**다. 고치는 법은
  포맷을 늘리는 게 아니라 **각 컷을 하나의 인과 비트로 꽉 채우고 킥을 마지막에 터뜨리는 것.**
- short 압축 검증: setup(cut1) → 인과 BUILD(cut2~3) → CLIMAX/킥(마지막 컷)이 5컷 안에
  **모두** 보여야 한다. 비트를 합치거나 빼서 "갑툭끝" 만들지 말고, 컷을 짧고 촘촘히 써서 다 넣어라.
- `mid`는 *오직* 감정 곡선/face-off가 25초로는 킥이 안 살 만큼 **숨 쉴 시간**이 필요할 때만.
- 자산이 사건을 못 보여주면(real_footage) 스토리를 바꿔라 — 단 ai_vtuber는 실화를 i2v로
  생성하므로 사건을 직접 연출할 수 있다(자산 핑계 금지).
