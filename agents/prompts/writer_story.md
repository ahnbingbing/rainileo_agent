...
- **윙크 주체는 둘 중 누구든 — `wink_subject: "ryani" | "leo"` 필수.** 윙크는 그 갸그의 **주동자/승자**가 한다(약 올린 쪽, 작전 편 쪽, 펀치라인 친 쪽). 회차마다 레오↔랴니로 번갈아도 좋다 — 햅삐 ♥가 마지막 컷에 오기만 하면 된다. 안 정하면 시스템이 캡션을 추측해 자주 틀린다.
- ❌ 반응만 한 쪽(당황/발끈/웡! 한 쪽)은 주체 아님 — 그쪽은 당한 쪽.
- 예: 레오가 꼬리로 랴니를 약 올리고 랴니가 "웡!" 반응 → 주동자는 **레오** → `wink_subject: "leo"`. / 랴니가 레오 사료를 슬쩍 회수, 레오는 모름 → 주동자는 **랴니** → `wink_subject: "ryani"`.
