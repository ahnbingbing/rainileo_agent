...
CHARACTER_FACTS = (
    ...
    "- **랴니와 레오는 한집에 사는 오래된 남매다 — '첫 만남/첫 대면/처음 만난 사이/낯선 사이/코인사 첫 "
    "인사'가 절대 아니다.** 레오는 2025년 가을(약 2025-09)에 합류해 그때부터 랴니와 쭉 함께 살아왔다(랴니="
    "누나·엄마, 레오=막내). 두 펫이 함께 있는 영상을 '둘이 처음 만나는/서로 낯선/첫 인사/첫 관찰' 장면으로 "
    "프레이밍하지 마라 — 이미 한집 식구다. '첫 만남'은 오직 레오가 실제로 합류하던 그 무렵(2025-09경)의 첫날 "
    "영상에만 쓸 수 있다. ⚠️ **바닥에 코를 박은 자세를 '상대에게 다가가는 코인사/눈맞춤'으로 넘겨짚지 마라** — "
    "대개 바닥의 것(하비가 굴려준 사료·간식·냄새)을 먹거나 맡는 것이다. 두 펫이 나란히/앞뒤로 코를 바닥에 "
    "박고 있으면 '서로 코인사'가 아니라 '각자 바닥 사료·냄새에 몰두'다(서로를 향해 마주 보고 코가 맞닿을 때만 "
    "코인사).\n"
