...
def _launch_intro_directive(today: str, render_style: str,
    ...
    # PD 2026-06-09: '어둡다'는 루미넌스가 아니라 **찍힌 시간(`tod`)**으로 본다.
    _day1_time = (" 첫날(Day1)은 **낮/노멀 시점(`tod`=낮)** 클립으로 — 늦은 저녁/밤(`tod`=저녁/밤) 클립은 피해라."
                  if offset == 0 else
                  " 저녁/밤(`tod`=저녁/밤) 클립도 OK — '저녁엔 이래요!' 식으로 시간대를 살려도 좋다(낮 클립과 섞을 땐 시점 명시).")
    inter = (" 런칭=소개 회차이니 둘이 뽀뽀·핥기·장난·나란히 붙기 등 **서로 상호작용**하는 "
             "순간을 우선 골라 관계를 보여줘라(솔로 컷 나열 금지). "
             "⚠️ 클립은 **둘이 한 프레임에 같이 있는(`both:true`) 클립을 최우선**으로(혼자 있는 클립 지양; "
             "기준작 ep_20260519: 둘이 나란히 걷고 붙어있음)." + _day1_time +
             " 캡션 톤은 **vlog 캐주얼 우대**"
             "(1인칭·친근, 동물농장 나레이터체보다), 단 단순한 묘사 금지 — 위트·속마음으로 풍부하게.")
    # EP1 '첫 만남'처럼 캐릭터가 직접 자기소개하는 포맷 — 두 레인 다 가능(PD 2026-06-07:
    # EP1-03도 사실 rf 실사인데 스티커 입히려 i2v를 거친 것뿐. rf=실제 클립 + 1인칭 펫
    # 자기소개 캡션, av=상상 연출 + 자기소개). 날짜별 주인공에 맞춘 자기소개 예시.
    _greet = {0: "'안녕! 나는 랴니예요(11살)', '나는 레오예요(10개월)' 둘 다 인사",
              1: "'안녕! 나는 레오예요! 10개월 장난꾸러기예요'",
              2: "'안녕! 나는 랴니예요! 11살 누나예요'"}.get(offset, "")
    ep1 = (f" ⚠️ 이 회차는 **반드시 캐릭터 자기소개 회차**다 (EP1 '첫 만남' 포맷): 캐릭터가 "
           f"자기 이름·나이·성격을 **직접 소개**한다({_greet}). **개그·사건·플롯 중심 금지** — "
           f"few-shot 예시(높은 점수 개그들)는 톤·리듬 참고용일 뿐, 반드시 자기소개 구조를 따르라. "
           f"제목/스토리가 '레오 회 도둑' 같은 개그면 런칭 자기소개가 아니다(틀림). "
           + ("rf=실제 클립에 **1인칭 펫 자기소개 캡션**을 얹어라(실제 펫이 인사하는 진정성)."
              if rf else "av=상상 연출로 캐릭터가 등장해 **자기소개**(개그 플롯 아님)."))
    if offset == 0:  # Day 1 — 둘 함께 채널 첫인사
        return ("[런칭 Day1 — 둘 함께 첫인사] 채널 첫 등장. 랴니(11살 의젓한 누나/엄마)와 "
                "레오(10개월 장난꾸러기 아들) **둘을 함께** 소개하는 따뜻한 첫인사 + 여름 시즌 "
                "가벼운 훅." + ep1 + inter)
    if offset == 1:  # Day 2 — 레오 집중
        return ("[런칭 Day2 — 레오 집중 소개] 오늘은 **레오** 중심. 장난꾸러기·사냥꾼·매복 전문, "
                "2025-11-15 떠돌이로 구조돼 랴니를 엄마로 여기는 origin. " + ep1 +
                ("**과거⇄현재 메모리레인**: 아기/구조 직후 레오 아카이브 클립(archive_videos) → "
                 "지금 10개월. 시점 명시 필수('구조된 날', '지금은'). 과거를 꺼내면 현재로 의미 연결."
                 if rf else " 레오의 성격·구조 origin을 상상 연출로. 랴니는 보조.") + inter)
    if offset == 2:  # Day 3 — 랴니 집중
        return ("[런칭 Day3 — 랴니 집중 소개] 오늘은 **랴니** 중심. 의젓·현명한 누나, "
                "**수영을 아주 잘하는(펠프스급) 면모**(물 공포 아님). " + ep1 +
                ("**과거⇄현재 메모리레인**: 과거 랴니 아카이브(archive_videos) → 지금 11살. "
                 "시점 명시 필수. 과거를 꺼내면 현재로 의미 연결. 물놀이 클립이 **실제 있을 때만** "
                 "수영 강조(없으면 물 얘기 억지로 넣지 마라)."
                 if rf else " 랴니의 의젓함·물 좋아하는 면을 상상 연출로. 레오는 보조.") + inter)
    return None
